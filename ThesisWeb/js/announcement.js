  ...
  fetch("announcement.php", {
    ...
    .then((data) => {
      if (!data.error) {
        ...
        // Construct the announcement HTML with conditional for announce_img
        announcementDiv.innerHTML = `
          <div class="announcement-title">${data.announce_title}
            <p class="announcement-timestamp">${data.timestamp}</p>
          </div>
          ${data.announce_img ? `<div class="announcement-img-body-div"><img class="announcement-img" src="${data.announce_img}"></div>` : ""}
        ...
        document.getElementById("announcements-container").prepend(announcementDiv); // Add new announcement to the top
